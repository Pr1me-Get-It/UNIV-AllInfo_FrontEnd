...
import { AlramContext } from '../data/Alram';  // : App에서 감싼 전역 상태(AlarmProvider)-> 알림 읽음/북마크 정보가 공유됨
import { api } from '../api/client';
import { ALRAM_DATA } from '../data/mockAlrams';  // 서버가 안 될 때 사용할 더미(mock) 데이터

const DEFAULT_IMAGE = require('../assets/knu.png');  // : 공지에 기본으로 붙일 이미지

// * export defailt : HomeScreen.jsx 모듈의 대표 컴포넌트. 외부 모튤에서 이 모듈을 불러오기하면, HomeScreen 컴포넌트가 시작점이 됨.

export default function HomeScreen({ navigation }) {      
    
    const { readStatus } = useContext(AlramContext) || { readStatus: {} };   // AlarmContext의 readStasus를 꺼내옴.  * useContext를 해야 정보를 꺼내올 수 있음!
                    ...
                    renderItem={({ item }) => {
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                onPress={() => navigation.navigate('Detail', { item })}  // Stack 구조에서, DetailScreen(name이 Detail인 Stack.Screen)이 Stack으로 쌓이게 됨.
                                                                                         // route.params 안에 { item } (이 객체) 가 들어가게 됨   -> route 객체를 만들어서 DetailScreen에 props로 넘겨주는 갓!
                            >
